...
export default function SEESuperAgent() {
  ...
  const executePlan = async () => {
    ...
    const clientEmail = basics.email || selectedClient?.email || '';
    ...
    const [calendarRows, reminderRows, taskRows, transportRows, supportRows, legalRows, riskRows] = await Promise.all([
      appointments.length ? base44.entities.CalendarEvents.bulkCreate(appointments.map((item) => ({ client_id: selectedClientId, client_email: clientEmail, user_email: clientEmail, plan_id: planId, title: item.title || item.event_title, event_title: item.event_title || item.title, event_type: item.event_type, date: item.date, start_time: item.start_time, end_time: item.end_time, location: item.location, notes: item.notes, reminder_time: '1 hour before', recurrence: item.recurrence, transportation_needed: !!item.transportation_needed, status: 'scheduled' }))) : Promise.resolve([]),
      ...
      tasks.length ? base44.entities.DailyTasks.bulkCreate(tasks.map((item) => ({ client_id: selectedClientId, client_email: clientEmail, user_email: clientEmail, plan_id: planId, task_title: item.task_title || item.title || item.name, task_category: item.task_category || item.category || 'recovery', description: item.description || item.notes || '', due_date: basics.discharge_date, due_time: item.due_time || '', recurrence: item.recurrence || 'weekly', priority: item.priority || 'medium', completed: false }))) : Promise.resolve([]),
