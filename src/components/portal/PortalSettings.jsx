    ...
    <div style={{ padding: "28px 28px 40px", maxWidth: 640, margin: "0 auto" }}>
      ...
      <Section title="Your Profile">
        ...
        <Field label="Email Address">
          <input value={user?.email || ""} readOnly
            style={{ ...INPUT_STYLE, background: "#F8FAFC", color: "#94A3B8", cursor: "not-allowed" }} />
        </Field>
        <Field label="Role">
          <input value={ROLE_LABELS[counselorProfile?.role_type] || counselorProfile?.role_type || "—"} readOnly
            style={{ ...INPUT_STYLE, background: "#F8FAFC", color: "#94A3B8", cursor: "not-allowed" }} />
        </Field>
        <Field label="Organization">
          <input value={counselorProfile?.facility_name || "—"} readOnly
            style={{ ...INPUT_STYLE, background: "#F8FAFC", color: "#94A3B8", cursor: "not-allowed" }} />
        </Field>
        ...
        <button onClick={() => save.mutate()} disabled={save.isPending}
          style={{ marginTop: 4, background: "#3B82F6", color: "#FFF", border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
      ...
      <Section title="Account">
        <button onClick={() => base44.auth.logout()}
          style={{ background: "#FEF2F2", border: "1px solid #FECACA", color: "#DC2626", borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Sign Out
        </button>
      </Section>
    </div>
